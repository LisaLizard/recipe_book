...
let pendingPhotoFile = null; // выбранный, но ещё не загруженный файл фото
...
const els = {
  ...
  photoInput: document.getElementById('field-photo-file'),
...
function loadIntoForm(id) {
  ...
  els.form.difficulty.value = recipe.difficulty || '';
...
function collectFormData() {
  ...
  return {
    ...
    photo_url: els.photoPreview.hidden ? '' : els.photoPreview.src,
  };
}
...
els.form.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    if (pendingPhotoFile) {
      data.photo_url = await uploadRecipePhoto(pendingPhotoFile, editingId);
    } else if (editingId) {
      // фото не менялось — оставляем то, что уже было в базе
      const existing = recipesCache.find((r) => String(r.id) === String(editingId));
